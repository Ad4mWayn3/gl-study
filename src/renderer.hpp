...
struct Uniform {
	GLuint id;
	T data;
	static Uniform create(T&& p_data, GLuint program, const char* name) {
		data = p_data;
...
struct Renderer {
	...
	static Renderer init(glm::ivec2 resolution);
	Renderer process(Seconds delta);
};
...
Renderer Renderer::init(glm::ivec2 resolution) {
	float square[] = {
		// front vertices
...
#pragma endregion
	...
	#define unif(x) Uniform<glm::mat4>::create(std::move(x), program.obj, #x);
	r.model = unif(model);
	r.view = unif(view);
	r.projection = unif(projection);
}
